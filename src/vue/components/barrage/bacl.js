(function( $ ) {
			...
			var BarrageItem = function(config){
				...
				this.config = config;
				//设置样式
				this.style = this.mySelf();
				//准备弹出去
				this.style.hide().appendTo(stage);
			}

			//单个弹幕样式,从config中提取配置
			BarrageItem.prototype.mySelf = function(){
				var style = $('<div style="min-width:400px;font-size:'+this.config.size +';color:'+this.config.color+';">'+this.config.text+'</div>');
	    	return style;
			}
